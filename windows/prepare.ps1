...
Start-Process C:\tmp\python-3.7.5.exe -ArgumentList '/quiet InstallAllUsers=1 PrependPath=1' -Wait;
...
# 3. install ant
wget "https://mirror.klaus-uwe.me/apache//ant/binaries/apache-ant-1.10.7-bin.zip" -OutFile "C:\tmp\ant.zip" ;
Expand-Archive -Path "C:\tmp\ant.zip" -DestinationPath "C:\tmp\";
Remove-Item "C:\tmp\ant.zip" -Force ;

# 3.1 set JAVA_HOME
$Env:ANT_HOME = "c:\tmp\apache-ant-1.10.7"
$Env:Path += ";c:\tmp\apache-ant-1.10.7\bin"
...
$Env:Path += ";c:\tmp\cmake-3.17.1-win64-x64\bin\cmake.exe"